 ...
 * the conversion characer % is replaced by the sequence % <code>n</code>$,
 * where <code>n</code> is a decimal integer giving the position of the argument
 ...
 * <table>
 * <tr>
 * <th align=left>Sequence</th> <th align=left>Name</th> <th
 * align=left>Description</th></tr> <tr><td>\\</td><td>backlash</td><td>None.
 * </td></tr> <tr><td>\a</td><td>alert</td><td>Attempts to alert the user
 * through audible or visible notification. </td></tr>
 ...
 * of a line. </td></tr> <tr><td>\f</td><td>form-feed</td><td>Moves the printing
 * position to the initial printing position of the next logical page.
 * </td></tr> <tr><td>\n</td><td>newline</td><td>Moves the printing position to
 * the start of the next line. </td></tr>
 * <tr><td>\r</td><td>carriage-return</td><td>Moves the printing position to the
 * start of the current line. </td></tr> <tr><td>\t</td><td>tab</td><td>Moves
 * the printing position to the next implementation- defined horizontal tab
 * position. </td></tr> <tr><td>\v</td><td>vertical-tab</td><td>Moves the
 * printing position to the start of the next implementation-defined vertical
 * tab position. </td></tr> </table></p> <h4>Conversion Specifications</h4>
 * <p>
 ...
 * <p>
 * The flags and their meanings are:</p> <dl> <dt>'<dd> integer portion of the
 * result of a decimal conversion (%i, %d, %f, %g, or %G) will be formatted with
 * thousands' grouping characters. For other conversions the flag is ignored.
 * The non-monetary grouping character is used. <dt>-<dd> result of the
 * conversion is left-justified within the field. (It will be right-justified if
 * this flag is not specified).</td></tr> <dt>+<dd> result of a signed
 * conversion always begins with a sign (+ or -). (It will begin with a sign
 ...
 * normally are. <dt>0<dd> d, i, o, x, X, e, E, f, g, and G conversions, leading
 ...
 * <p/>
 * <p>
 * The conversion characters and their meanings are: </p> <dl> <dt>d,i<dd>The
 * int argument is converted to a signed decimal in the style [-]dddd. The
 ...
 * explicit precision of 0 is no characters. <dt>o<dd> The int argument is
 * converted to unsigned octal format in the style ddddd. The precision
 ...
 * leading zeros. The default precision is 1. The result of converting 0 with an
 * explicit precision of 0 is no characters. <dt>x<dd> The int argument is
 * converted to unsigned hexadecimal format in the style dddd; the letters
 * abcdef are used. The precision specifies the minimum numberof digits to
 * appear; if the value being converted can be represented in fewer digits, it
 * will be expanded with leading zeros. The default precision is 1. The result
 * of converting 0 with an explicit precision of 0 is no characters. <dt>X<dd>
 * Behaves the same as the x conversion character except that letters ABCDEF are
 * used instead of abcdef. <dt>f<dd> The floating point number argument is
 * written in decimal notation in the style [-]ddd.ddd, where the number of
 * digits after the radix character (shown here as a decimal point) is equal to
 * the precision specification. A Locale is used to determine the radix
 * character to use in this format. If the precision is omitted from the
 * argument, six digits are written after the radix character; if the precision
 * is explicitly 0 and the # flag is not specified, no radix character appears.
 * If a radix character appears, at least 1 digit appears before it. The value
 * is rounded to the appropriate number of digits. <dt>e,E<dd>The floating point
 * number argument is written in the style [-]d.ddde{+-}dd (the symbols {+-}
 * indicate either a plus or minus sign), where there is one digit before the
 * radix character (shown here as a decimal point) and the number of digits
 * after it is equal to the precision. A Locale is used to determine the radix
 * character to use in this format. When the precision is missing, six digits
 * are written after the radix character; if the precision is 0 and the # flag
 * is not specified, no radix character appears. The E conversion will produce a
 * number with E instead of e introducing the exponent. The exponent always
 * contains at least two digits. However, if the value to be written requires an
 * exponent greater than two digits, additional exponent digits are written as
 * necessary. The value is rounded to the appropriate number of digits.
 ...
 * to the end of the string are written. <dt>%<dd>Write a % character; no
 * argument is converted. </dl>
 * <p>
...
public class PrintfFormat {
    ...
    public String sprintf(final Object... o) {
        ...
        for (final ConversionSpecification cs : vFmt) {
