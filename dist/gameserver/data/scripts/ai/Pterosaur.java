...
public class Pterosaur extends DefaultAI {
    ...
    public Pterosaur(final NpcInstance actor) {
        super(actor);
        current_point = -1;
        wait_timeout = 0L;
        wait = false;
    ...
    protected boolean thinkActive() {
        ...
        if (now > wait_timeout && (current_point > -1 || Rnd.chance(5))) {
            ...
            wait = true;
            ++current_point;
            if (current_point >= Pterosaur.points.length) {
                current_point = 0;
            }
            addTaskMove(Pterosaur.points[current_point], false);
            doTask();
            return true;
        }
