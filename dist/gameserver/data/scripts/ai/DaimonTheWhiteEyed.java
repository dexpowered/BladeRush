...
public class DaimonTheWhiteEyed extends DefaultAI {
    ...
    public DaimonTheWhiteEyed(final NpcInstance actor) {
        super(actor);
        current_point = -1;
        wait_timeout = 0L;
        wait = false;
    ...
    protected boolean thinkActive() {
        ...
        if (!wait && current_point == 23) {
            wait_timeout = System.currentTimeMillis() + 5000L;
            return wait = true;
        }
        wait_timeout = 0L;
        wait = false;
        ++current_point;
        if (current_point >= DaimonTheWhiteEyed.points.length) {
            current_point = 0;
        }
        addTaskMove(DaimonTheWhiteEyed.points[current_point], true);
        doTask();
        return true;
    }
